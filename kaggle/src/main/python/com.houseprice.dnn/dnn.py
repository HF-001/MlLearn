...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = "99"#打印运行日志

from tensorflow.python.client import device_lib
device_lib.list_local_devices()#输出设备信息
...
with tf.device('/cpu:0'): # 指定在cpu上运行
...
from __future__ import print_function#强制按python3的语法写
...
from sklearn.preprocessing import MinMaxScaler#最简单的归一化

tf.logging.set_verbosity(tf.logging.INFO)#设置日志级别
sess = tf.InteractiveSession()#开启会话，允许动态添加计算图

train = pd.read_csv('../other/kaggle/train.csv')
print('Shape of the train data with all features:', train.shape)
train = train.select_dtypes(exclude=['object'])#根据类型进行选取
print("")
print('Shape of the train data with numerical features:', train.shape)
train.drop('Id',axis = 1, inplace = True)#删除id列，会改变源数据
train.fillna(0,inplace=True)#补0

test = pd.read_csv('../other/kaggle/test.csv')
...
# Model
tf.logging.set_verbosity(tf.logging.ERROR)#设置日志级别
...
for continuous_feature in FEATURES:#处理连续特征
    engineered_features.append(
        tf.contrib.layers.real_valued_column(continuous_feature))

for categorical_feature in FEATURES_CAT:#处理离散特征
    sparse_column = tf.contrib.layers.sparse_column_with_hash_bucket(#构建低维离散特征
